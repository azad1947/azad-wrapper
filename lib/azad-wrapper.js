...
const update_url_with_pathparam = (options, req) => {
    const url = options.path_params.params.reduce((acc, val) => {
        if (options.path_params.value_from.toLowerCase() === 'body') {
            if (options.path_params.map) {
                acc = acc.replace(`{${val}}`, req.body[options.path_params.map[val]]);
            } else {
                acc = acc.replace(`{${val}}`, req.body[val]);
            }
        }

        if (options.path_params.value_from.toLowerCase() === 'query') {
            if (options.path_params.map) {
                acc = acc.replace(`{${val}}`, req.query[options.path_params.map[val]]);
            } else {
                acc = acc.replace(`{${val}}`, req.query[val]);
            }
        }
...
const make_api_call = (options, req, res, next) => {
    ...
    axios(options)
        .then((response) => {
            ...
            if (options.response_map && !_.isEmpty(options.response_map)) {
...
exports.error_handler = (env = 'prod', msg = null) => {
    return (err, req, res, next) => {
        ...
        if (env.toLocaleLowerCase() === 'dev') {
            ...
            error.message = msg ? msg : 'failed to make http request. plz try again.';
        }
